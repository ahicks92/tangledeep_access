...
namespace TangledeepAccess.Dev {
    /// <summary>
    /// Minimal loopback HTTP/1.1 server on a raw TcpListener. We speak just enough
    /// of the protocol for curl (request line, Content-Length body, one response,
    /// connection close) and deliberately avoid System.Net.HttpListener so there is
    /// no http.sys URL-ACL/admin requirement. Loopback-only; dev tool, not hardened.
    /// </summary>
    internal sealed class DevHttpServer {
        ...
        public void Start() {
            ...
            _listener.Start();
            _running = true;
            _thread = new Thread(Loop) { IsBackground = true, Name = "TDDevHttp" };
            _thread.Start();
        }
        ...
        private void Handle(TcpClient client) {
            ...
            while (headerEnd < 0) {
                ...
                if (data.Count > 1 << 20) {
                    Write(stream, 400, "header too large\n");
                    return;
                }
            }
            ...
            string response;
            try {
                response = _handler(method, path, body) ?? "";
            } catch (Exception e) {
                Write(stream, 500, "handler error: " + e + "\n");
                return;
            }
